...
def dist_softmax(fake_res):
    ...
    for i in range(1, fake_res.size(0)):
        res = fake_res[i]
        reses = res.expand(args.anchors, args.resSize)
        dists = nn.functional.pairwise_distance(reses, anchors, p=2)
...
def plot_with_labels(embeddings, labels, title, path):
    ...
    for x, y, s in zip(X, Y, labels):
        if s == 0:
            ...
            shape = 's'
        c = cm.rainbow(int(s*100))
        plt.scatter(x, y, color=c, marker=shape, s=20)
...
for epoch in range(args.nepoch):
    ...
    for i in range(0, data.ntrain, args.batch_size):
        ...
        G_cost = (-(criticG_fake + criticG_fake_UD)) + \
            args.delta * cls_loss + dis_loss + map_loss
        G_cost.backward()
        optM.step()
        optG.step()
        optC.step()
...
f.close()
#np.savetxt("test_result.txt", max(results),delimiter=',', fmt='%6.4f')
